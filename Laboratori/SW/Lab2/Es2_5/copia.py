...
class Catalog():
    ...
    def get_mqtt(self, uri, params):
        ...
        if uri[2] == "messagebroker":
            self.myMqttClient.myPublish("/tiot/16/GET/messagebroker/response",
                                        (json.dumps(self.messagebroker, indent=4)))

        elif uri[2] == "devices":
            if uri[0] == 5:
                self.myMqttClient.myPublish("/tiot/16/GET/devices/response",
                                            (json.dumps({"devices": self.devices}, indent=4)))
            else:
            ...
            if uri[0] == 5:
                self.myMqttClient.myPublish("/tiot/16/GET/users/response",
                                            (json.dumps({"users": self.users}, indent=4)))
            else:
            ...
            if uri[0] == 5:
                self.myMqttClient.myPublish("/tiot/16/GET/services/response",
                                            (json.dumps({"services": self.services}, indent=4)))
            else:
